...
export default function HomePage() {
    ...
    if (loading) {
        return (
            <div style={{ backgroundColor: '#22c55e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <p>Cargando...</p>
            </div>
        );
    }
    ...
    return (
        <div style={{ backgroundColor: '#22c55e', minHeight: '100vh', padding: '20px' }}>
            ...
            {matches.length === 0 ? (
                <p style={{ textAlign: 'center', fontSize: '14px' }}>No hay partidos disponibles</p>
            ) : (
                    ...
                    {matches.map((match) => (
                        <div key={match.id} style={{ backgroundColor: '#9ca3af', border: '3px solid black', padding: '15px', boxShadow: '5px 5px 0 rgba(0,0,0,0.3)' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                                <input type="number" min="0" placeholder="0" style={{ flex: 1, padding: '8px', border: '2px solid black', textAlign: 'center', fontSize: '12px' }} />
                                <span style={{ display: 'flex', alignItems: 'center', fontSize: '14px' }}>-</span>
                                <input type="number" min="0" placeholder="0" style={{ flex: 1, padding: '8px', border: '2px solid black', textAlign: 'center', fontSize: '12px' }} />
                            </div>

                            <button style={{ width: '100%', padding: '10px', backgroundColor: '#22c55e', border: '3px solid black', fontSize: '11px', cursor: 'pointer', fontWeight: 'bold' }}>GUARDAR</button>
                        </div>
                    ))}
                </div>
            )}
